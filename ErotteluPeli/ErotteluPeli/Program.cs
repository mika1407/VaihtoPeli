    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                // Vaihdetaan muuttujat 1 ja 2 , iteroidaan lista
                for (int i = 0; i < lista.Count; i++)
                {
                    if (lista[i] == vaihdaYksi)         // jos vaihdaYksi on listalla ja halutaan vaihtaa
                        lista[i] = vaihdaKaksi;         // Asetaan vaihda1 vaihdaKaksi muuttujan arvoksi
                    else if (lista[i] == vaihdaKaksi)   // jos taas numero jota katsomme on vaihdaKaksi
                        lista[i] = vaihdaYksi;          // annetaan vaihdaYksi muuttuajalle arvo vaihda2
                }
